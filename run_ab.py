        ...
        # for gammaG in [gammaD]: ##TODO
            for seed_value in seed_values:
              for recon in recon_values:
                # 构建日志文件路径
                log_file_path = f"log/AB/nofeedback.log"
                ...
                --gzsl --recons_weight {recon} --a1 0 --a2 0 --feed_lr 0.0000 --dec_lr 0.000 --feedback_loop 2'''
                ...
                log_file_path = f"log/AB/nofeedback_cls.log"
                if os.path.exists(log_file_path):
                    print(f"Log file {log_file_path} already exists. Skipping...")
                    continue
                ...
                # 构建命令字符串
                command = f'''CUDA_VISIBLE_DEVICES=0 OMP_NUM_THREADS=4 python train_images.py --gammaD {gammaD} --gammaG {gammaG} \
                --manualSeed {seed_value} --encoded_noise --preprocessing --cuda --image_embedding SqueezeNet_512_T --class_embedding Video_1024_gzsl \
                --nepoch 300 --ngh 4096 --ndh 4096 --lr {lr} --classifier_lr {lr*10} --lambda1 10 --critic_iter 5 --dataroot datasets --dataset RADAR \
                --nclass_all 27 --batch_size 128 --nz 1024 --latent_size 1024 --attSize 1024 --resSize 512 --syn_num 320 \
                --gzsl --recons_weight {recon} --a1 0 --a2 0 --feed_lr 0.00001 --dec_lr 0.0001 --feedback_loop 2'''
                ...
                log_file_path = f"log/AB/cls.log"
                if os.path.exists(log_file_path):
                    print(f"Log file {log_file_path} already exists. Skipping...")
                    continue
